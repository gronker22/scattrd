...
enum VillainAnalysis {
    static func detect(store: Database, now: Date = Date()) -> Villain {
        ...
        while day <= today {
            ...
            let blocks = FocusScore.analyze(store.sessions(from: s, to: e)).blocks
            for i in blocks.indices where i > 0 {
                totalSwitches += 1
                let b = blocks[i]
                if b.category == .communication || b.category == .distraction {
                    switchIns[b.app, default: 0] += 1
                }
            }
        ...
        guard totalSwitches > 0, !switchIns.isEmpty else {
            return Villain(valid: false, app: "—", switchIns: 0, pct: 0, totalSwitches: totalSwitches,
                           runnerUp: "—", runnerUpPct: 0, periodLabel: label)
        }
        let sorted = switchIns.sorted { $0.value > $1.value }
        let top = sorted[0]
        let pct = Int((Double(top.value) / Double(totalSwitches) * 100).rounded())
        let runner = sorted.count > 1 ? sorted[1] : nil
        let rPct = runner.map { Int((Double($0.value) / Double(totalSwitches) * 100).rounded()) } ?? 0
        return Villain(valid: true, app: top.key, switchIns: top.value, pct: pct, totalSwitches: totalSwitches,
                       runnerUp: runner?.key ?? "—", runnerUpPct: rPct, periodLabel: label)
    }
...
if(!V.valid){
  ...
  root.innerHTML = brand +
    '<div class="eyebrow">App Villain</div>'+
    ...
    '<div class="charge">caused <b>'+V.pct+'%</b> of your context switches this month</div>'+
    '<div class="stats">'+
    ...
    '</div>'+
    (V.runnerUp!=='—'?'<div class="runner">Runner-up: <b>'+V.runnerUp+'</b> ('+V.runnerUpPct+'%)</div>':'')+
